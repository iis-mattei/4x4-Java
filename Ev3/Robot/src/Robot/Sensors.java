...
//	private EV3UltrasonicSensor usSide; // S3
//	private SampleProvider spSide;
//	private float[] sampleSide;
...
//		spSide = usSide.getDistanceMode();
		//sampleFwdHigh = new float[spFwdHigh.sampleSize()];
//		sampleSide = new float[spSide.sampleSize()];

		arduino = new I2CSensor(SensorPort.S4, I2C_SLAVE_ADDRESS);
	}
	...
	public boolean isAcrossLine() {
		return (colorC.equals("b") && getColorL().equals("w") && getColorR().equals("w"));
	}
	...
	public void checkTouches() {
		...
		if (Main.DEBUG) {
			System.out.println(
					"Tocchi: " + touchFwdRight + " " + touchFwdLeft + " " + touchBackRight + " " + touchBackLeft);
		}
	}
...
//		spSide.fetchSample(sampleSide, 0);
//		int dist = (int) Math.round(sampleSide[0] * 100);
//		if (Main.DEBUG) {
